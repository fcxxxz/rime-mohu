...
os.remove(root)
assert(os.execute("mkdir -p " .. root .. "/mohu_llm/config " .. root .. "/mohu_llm/models/Qwen3.5-0.8B-MLX-4bit") == true)
assert(os.execute("mkdir -p " .. root .. "/mohu_llm/models/Qwen3-0.6B-4bit") == true)
...
local models = catalog.list({ user_data_dir = root })
...
assert(models[1].relative_path == "mohu_llm/models/Qwen3.5-0.8B-MLX-4bit")
...
write(root .. "/mohu_llm/config/model-selection", "qwen3-0.6b\n")
local unavailable = catalog.status({ user_data_dir = root })
assert(unavailable.selection_id == "qwen3-0.6b")
assert(unavailable.status == "unavailable")

write(root .. "/mohu_llm/models/Qwen3-0.6B-4bit/config.json", "{}\n")
...
write(root .. "/mohu_llm/models/Qwen3-0.6B-4bit/tokenizer.json", "{}\n")
write(root .. "/mohu_llm/models/Qwen3-0.6B-4bit/config.json",
...
assert(available.model.relative_path == "mohu_llm/models/Qwen3-0.6B-4bit")
...
write(root .. "/mohu_llm/config/model-selection", "made-up-model\n")
...
os.remove(root .. "/mohu_llm/config/model-selection")
