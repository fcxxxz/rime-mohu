...
int main() {
  ...
  const Case cases[] = {
      // 只吃 不是码表词，支持 是 → top1=只吃… 分歧=1（旗舰形态）
      {"这个输入法只吃一口气输入一整句话", "这个输入法支持一口气输入一整句话", 1,
       "只吃(top1非词)/支持(top2词) 开门"},
      // 反序：top1=支持（词）→ 不开门
      {"这个输入法支持一口气输入一整句话", "这个输入法只吃一口气输入一整句话", 0,
       "支持(top1词) 不开门"},
      // 暴力 与 暴利：暴力 是词 → top1=暴力 不开门；top1=暴利(非词) 开门
      {"你觉得什么是暴力行业", "你觉得什么是暴利行业", 0, "暴力(top1词) 不开门"},
      {"你觉得什么是暴利行业", "你觉得什么是暴力行业", 1, "暴利(top1非词) 开门"},
      // 无分歧（前缀耗尽其一）
      {"支持", "支持", 0, "相同候选 不开门"},
  };
  bool ok = true;
