...
--  0.2.0: 增加快速切换切分的能力，因而从 mohu_semicolon_processor 更名为 mohu_processor
...
local function semicolon_processor(key_event, env)
    ...
    while i < page_size do
        ...
        if mohu.unicode_code_point_is_chinese(codepoint) -- 汉字
...
local function steal_auxcode_processor(key_event, env)
    ...
    if stealee:has_tag("_mohu_stealee") then
        ctx.input = ctx.input:sub(1, stealer._start) .. ctx.input:sub(stealer._start + 2)
        stealee.tags = stealee.tags - Set({"_mohu_stealee"})
        return kAccepted
    end
...
local shorthands = {
    ...
    [string.byte("V")] = function(_, s)
        return s .. "着" .. s .. "着"
    end,
    [string.byte("Q")] = function(_, s)
        return s .. "来" .. s .. "去"
    end,
}
...
return {
    init = function(env)
        ...
        if cfg:get_bool("mohu/shorthands") then
