...
- **2026-09-16 V5 模型 u8 量化替换发布（TCSKNM04）**：V5（573MB）的后验
  剪枝存在均匀损失墙（两码/纯双拼口径约 −1pp/45MB，tau/ratio/topk/floor 同
  斜率），不可作为缩体积手段；量化才是无损手段。当日两步落地：f16
  （TCSKNM03，后继 6B）先验证并临时部署，u8 对数码（TCSKNM04，p=
  exp(−14+k×14/255)，后继 5B，256 项查找表还原）全量验证同样持平
  （±0.02pp、两码 held 逐位一致、魔虎纯双拼全量 65.47% 反超 f32 的
  65.39%、四模式三项反超）后**作为正式发布**：`mohu-sentence-ngram-v5.bin`
  = u8 内容（389.7MB，−32%，驻留 536MB/−208MB，延迟持平）；标准 float8
  （e5m2）因指数位浪费只剩 2 位尾数（±12.5%）实测有损失——**1 字节方案
  的正确形态是对数码不是 float8**。引擎 02/03/04 三格式兼容（magic 分发、
  `succ_entry_bytes` 参数化，旧引擎干净拒绝并回退），剪枝/量化工具
  `tools/prune_tiger_ngram.cc`（`--format u8|f16`），评测
  `research/tiger2code_bench/eval_model.py|eval_cases.py`；V5 的 λ 是插值
  权重**不是** 1−Σp（剪枝必须保 λ 原值、只加回剪除质量，no-addback 实测
  −2pp）。完整扫描表、三模型对照与部署记录见
...
N 条，深尾部不再显示（0.3.0）。**同日 0.4.0 移除 ≤4 字 _personal
免配额豁免**：引擎 personal 标记是路径级继承（解码状态沿边传播），
qygfda 变体全走「跟打」共享个人边、整族 `_personal`，豁免把上限整个
吞掉（用户实测部署后仍刷屏；0.3.0 单测只建模 1 条 personal 故未拦）。
学习闭环不断：排第一的学习词占前排配额位（个人词先验保证），同族
兄弟进押后尾部仍可选（xspizi 熊皮子/熊罴子不受损）。只裁显示层——
引擎评分池、重排、学习层不受影响；辅码消歧是深尾部变体的正规选择路径。重排链看的候选池
...
句形配额（用户词库随 native 序输出；**2026-09-16 0.4.0 起该豁免
移除**，短 personal 同入配额、押后仍可选）；反复输入的长句 personal 仍
...
- **字符级模型看不见词界（2026-09-16 补，文本词典先验）**：词边先验按
  「编码内部边」投票，管不到「真词只挂简码、不挂全码」的场景——
  `tsyige` 的「同一个」（词表只有简码 tyg 条目）在「实现」类上文下被
  「统一个」（统一+个 拼装）反杀，两条路径共享「一个」边、词边加成对称
  抵消。修复：`tiger/text_lexicon_weight`（默认 6.5，0 关闭，clamp
  [0,16]；初版 1.5 不足——强搭配上文实测：和平 4.52 / 祖国 3.31 /
  大一 2.27 nats 的 P(统|ctx) 杠杆，12 组上下文 6.5 全翻正）
  ——候选**完整文本**命中词表多字条目（任意码形）时，输出分与提前上屏
  置信度加该有界分；只作用输出层（to_out），beam 按文本去重不变。4 键
  排序不变性保持（≤4 键按 edges→rank 排序，加分不改变结构）。与个人词
  先验正交：个人词按提交计数、无词表要求，此项按词表存在性；OOV 词由
  个人词路径追回（12+1.5 > 6.5，两次提交反超）。详见
  [文本词典先验报告](../reports/2026-09-16-text-lexicon-prior.md)。
...
- **词边累加缺陷（V6 待解，2026-09-24）**：`word_edge_weight` 每条 +1.5、
  无上限，长句里按词计数累加会淹没字符模型有界的局部偏好（每分岔至多
  1~2 nats）——名句前缀 `yewufgyuyewu` 被「业务风雨业务」以 3 条边 +4.5
  对纯三元偏好 +1.74 反杀（分差精确闭合见报告表）。受害类＝长句内含
  词典未收录（或仅注入、rank 99 无内部边）的二字序列且局部偏好 <1.5；
  诗句歌词引语内嵌全中。豁免＝全码整段命中（word_form）、局部偏好 >1.5
  的分岔、个人词内部边（boost ≤12）。**V6 处置**：模型自带词界证据后
  整体重标定/下调 word_edge；封顶/边际递减方案会同步压缩真句的词边
  优势（重开粘连类），须 19,996 句基准才可评估；词频加权边是反方向；
  前缀词联想已被用户否决——**候选第一必须对应已输入音节**，未打的
  音节不上桌。详见
  [≥4 字注入报告追加节](../reports/2026-09-24-long-word-fullcode-injection.md)。
