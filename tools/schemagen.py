...
auxiliary_code_choices = ['tiger_prefix2', 'zrm', 'user']
...
def to_auxiliary_codes(char):
    ...
    if not auxiliary_table:
        match args.auxiliary_code:
            case 'tiger_prefix2' | 'zrm':
                auxiliary_table = read_zrmdb_table('tools/data/tiger_aux.txt')
            case 'user':
...
def iter_char_codes(char, pinyin):
    try:
        ...
        auxiliary_codes = to_auxiliary_codes(char)
        if not auxiliary_codes:
            if args.auxiliary_code_fallback:
                auxiliary_codes = [args.auxiliary_code_fallback]
            else:
                raise KeyError(f'Missing auxiliary code for {char}')
...
parser = argparse.ArgumentParser(
    prog='schemagen',
    description='Mohu-like Rime schema+dictionary generator',
...
parser.add_argument('--auxiliary-code', choices=auxiliary_code_choices, help='輔助碼方案', default='tiger_prefix2')
parser.add_argument('--auxiliary-code-fallback', help='若無輔助碼則使用該fallback')
