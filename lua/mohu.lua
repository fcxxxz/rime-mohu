...
function Module.open_rime_file(rel_path, pathsep)
    ...
    if file then
        ...
        log.error('mohu: failed to open ' .. path .. ', error: ' .. err)
    end
...
---Load zrmdb.txt bundled with the standard Mohu distribution.
...
---Load the first-occurrence Tiger rank bundled with this schema.
---@return table<integer,integer>?
function Module.load_tiger_rank()
    if Module.tiger_rank then
        return Module.tiger_rank
    end
    local pathsep = (package.config or '/'):sub(1, 1)
    local file = Module.open_rime_file('lua' .. pathsep .. 'tiger_rank.txt', pathsep)
    if not file then
        log.error('mohu: failed to open Tiger rank file')
        return nil
    end
    local ranks = {}
    for line in file:lines() do
        local char, raw_rank = line:match("^(.+)\t(%d+)$")
        local codepoint = char and utf8.codepoint(char)
        local rank = tonumber(raw_rank)
        if codepoint and rank then
            ranks[codepoint] = rank
        end
    end
    file:close()
...
function Yielder.new(before_cb, after_cb)
    local instance = {
        ...
        index = 0,
        before_cb = before_cb, -- function(number,Cand): number?, 在准备 yield 前让用户检查一次，如果不应该此时 yield，则返回一个 defer 数字
        after_cb = after_cb  -- 真正 yield 后通知用户
    }
...
---@return table? 新得到的应该输出的 deferred value
...
---输出 value，同时可能会输出之前被延迟的value，导致输出不止一个结果。
---@param value Candidate
