...
--
-- 0.3.0: Delegate reordering to mohu_reorder_filter.
--
-- 0.2.0: Add `fix_use_dict` option to use "mohu_fixed", just like "mohu".
...
function Top.init(env)
    env.fix_use_dict = env.engine.schema.config:get_bool("mohu/fix/use_dict") or false
    if env.fix_use_dict then
        env.fixed = Component.Translator(env.engine, "", "table_translator@fixed")
        env.use_dict_max_length = env.engine.schema.config:get_int("mohu/fix/use_dict_max_length") or 3
    end
    env.cache = {}
end
...
function Top.get_needle(env, input)
    ...
    local val = env.engine.schema.config:get_string("mohu/fix/" .. input)
    env.cache[input] = val
    return val
end
