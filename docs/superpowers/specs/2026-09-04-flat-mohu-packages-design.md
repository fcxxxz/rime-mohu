...
V5 n-gram 模型不进入方案包，作为独立的 `mohu-model-v5.zip` 发布。该模型包的根路径为 `mohu/model/sentence-ngram-mobile.bin`，用户解压到 Rime 用户目录即可完成模型放置。
...
`default.yaml` 只注册当前包的公开 schema；自然码包不注册小鹤，小鹤包不注册自然码。方案包不包含 `mohu/model/sentence-ngram-mobile.bin`，但保留模型目标目录信息和缺失时的 fail-open 行为。

模型包只包含：

```text
mohu-model-v5.zip
└── mohu/model/sentence-ngram-mobile.bin
```

模型文件使用固定名称和 CI 校验的 SHA-256。模型包可独立更新，不触发方案包重打包。

## 构建与发布

Makefile 增加扁平方案包目标，先将方案内容直接复制到目标根目录，再压缩；不再使用现有 LLM `base/` 安装包目标作为 Release 入口。GitHub Actions 只发布两个扁平方案 zip 和一个独立 V5 模型 zip，删除旧普通包和带 `llm` 名称的 Release 资产。
...
4. 模型包只包含固定的 `mohu/model/sentence-ngram-mobile.bin`；
...
- `mohu/model/sentence-ngram-mobile.bin` 缺失时 native 引擎回退普通候选；
