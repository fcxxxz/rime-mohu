...
### 模型格式：TCSKNM04（u8 对数后继概率，体积 -32%）

2026-09-16 起，`mohu-sentence-ngram-v5.bin` 的发布内容为 **u8 量化版**
（TCSKNM04 格式：后继概率以 1 字节对数码存储 p=exp(−14+k×14/255)，
每条 8→5 字节，389.7MB，较原 f32 版 573.1MB −32.0%）。量化无损——
虎码压力口径两码 89.39%/最优码 99.56% 与 f32 持平（±0.02pp，两码
held-out 逐位一致）；魔虎方案口径纯双拼整句全量 19,996 句 65.47% 高于
f32 的 65.39%，四模式 ±0.1pp 内；mmap 驻留内存随体积同比例下降（同负载
实测峰值 RSS 744MB→536MB，−208MB）；逐键延迟经交替隔离基准验证不变
（8 档输入长度 Δ −0.7%~+0.6%，无方向性）。完整论证、剪枝扫描与三模型
对照见 `docs/reports/2026-09-16-v5-f16-quantize.md`。

安装方式不变：文件放入 `~/Library/Rime/mohu/model/` 即可（运行时固定读取
`mohu-sentence-ngram-v5.bin` 文件名）。**TCSKNM04 需要 2026-09-16 起构建
的 `libtigerengine`**（方案包内 `mohu/runtime/`）：旧引擎读 04 文件报
`unknown model format` 并按既有回退逻辑退回普通候选，升级模型请同步更新
方案包。旧 f32 版模型保留备份的话可直接改回原名回滚（02 格式新引擎同样
支持）。`tools/prune_tiger_ngram.cc` 可从任一 02 模型再生成 03（f16）或
04（u8）文件，并支持阈值剪枝（tau/ratio/topk/floor/ctx-mass 旋钮的
体积-精度权衡见报告）。

### TCSKNM02/03/04 页校验与损坏模型回退

TCSKNM02/03/04 默认只在启动时校验文件头、分区算术、索引计数/键序和页起始范围；
